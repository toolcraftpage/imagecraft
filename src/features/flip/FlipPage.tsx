...
export default function FlipPage() {
  ...
  return (
    <>
      <Helmet><title>Flip Image – ImageCraft</title></Helmet>
      <Container className="py-12">
        <h1 className="mb-8 text-3xl font-bold text-gray-900 dark:text-white">Flip Image</h1>
        <ImageDropZone images={images} onDrop={handleDrop} onFileInput={handleFileInput} fileInputRef={fileInputRef} onClear={clearImages} />
        {images.length > 0 && !images[0].error && <FlipControls image={images[0]} />}
      </Container>
    </>
  );
}