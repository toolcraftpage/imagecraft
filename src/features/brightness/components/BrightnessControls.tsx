...
export default function BrightnessControls({ image }: { image: ImageFile }) {
  ...
  useEffect(() => { apply(); }, [brightness, img]);

  const download = () => {
    if (previewUrl) { const a = document.createElement('a'); a.href = previewUrl; a.download = `brightness-${image.file.name}`; a.click(); }
  };
  ...
  return (
    <div className="mt-8 space-y-6 rounded-card border p-6">
      ...
      <div>
        <label className="mb-2 block text-sm">Brightness ({brightness})</label>
        <input type="range" min="-100" max="100" value={brightness} onChange={e => setBrightness(Number(e.target.value))} className="w-full accent-primary-500" />
      </div>
      {previewUrl && (
        <div>
